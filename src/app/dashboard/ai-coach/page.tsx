...
const QUICK_PROMPTS = [
  ...
  'Review my psychology this month',
...
export default function AICoachPage() {
  ...
  return (
    <div style={{ maxWidth: '900px', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 130px)' }}>
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ color: 'white', fontWeight: '800', fontSize: '22px', margin: '0 0 4px 0' }}>🤖 AI Coach</h1>
        <p style={{ color: '#475569', fontSize: '13px', margin: 0 }}>Powered by Groq • Personalised to your trading data</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
        {QUICK_PROMPTS.map(p => (
          <button key={p} onClick={() => send(p)}
            style={{
              padding: '6px 12px', borderRadius: '20px', border: '1px solid rgba(59,130,246,0.25)',
              background: 'rgba(59,130,246,0.05)', color: '#60a5fa', fontSize: '12px',
      ...
      <div style={{
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
            <div style={{
              ...
              background: msg.role === 'assistant' ? 'linear-gradient(135deg, #3b82f6, #6366f1)' : 'rgba(255,255,255,0.1)',
            }}>
              {msg.role === 'assistant' ? '🤖' : '👤'}
            </div>
            <div style={{
        ...
        {loading && (
          <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #3b82f6, #6366f1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🤖</div>
            <div style={{ padding: '12px 16px', borderRadius: '16px 16px 16px 4px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ display: 'flex', gap: '4px' }}>
                {[0, 1, 2].map(i => (
                  <div key={i} style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#3b82f6', animation: 'bounce 1.2s infinite', animationDelay: `${i * 0.2}s` }} />
                ))}
              </div>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        <input
          ...
          style={{
            flex: 1, padding: '13px 16px', background: 'rgba(255,255,255,0.05)',
            border: '1px solid rgba(59,130,246,0.25)', borderRadius: '10px',
